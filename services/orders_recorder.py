...
class OrdersRecorder:
    ...
    def __init__(self, db_manager: AsyncDatabaseManager, account_name: str, connector_name: str):
        ...
        self._create_order_forwarder = SourceInfoEventForwarder(
            self._did_create_order)
        self._fill_order_forwarder = SourceInfoEventForwarder(
            self._did_fill_order)
        self._cancel_order_forwarder = SourceInfoEventForwarder(
            self._did_cancel_order)
        self._fail_order_forwarder = SourceInfoEventForwarder(
            self._did_fail_order)
        self._complete_order_forwarder = SourceInfoEventForwarder(
            self._did_complete_order)
    ...
    async def stop(self):
        ...
        logger.info(
            f"OrdersRecorder stopped for {self.account_name}/{self.connector_name}")
    ...
    def _did_create_order(self, event_tag: int, market: ConnectorBase, event: Union[BuyOrderCreatedEvent, SellOrderCreatedEvent]):
        ...
        try:
            trade_type = TradeType.BUY if isinstance(
                event, BuyOrderCreatedEvent) else TradeType.SELL
    ...
    async def _handle_order_created(self, event: Union[BuyOrderCreatedEvent, SellOrderCreatedEvent], trade_type: TradeType):
        ...
        try:
            async with self.db_manager.get_session_context() as session:
                ...
                if existing_order:
                    ...
                    if existing_order.status in ["PENDING_CREATE", "PENDING", "SUBMITTED"]:
                        ...
                        logger.info(
                            f"OrdersRecorder: Updated status to OPEN for order {event.order_id}")
            ...
            logger.debug(f"Recorded order created: {event.order_id}")
        except Exception as e:
            logger.error(f"Error recording order created: {e}")
        ...
        try:
            async with self.db_manager.get_session_context() as session:
                ...
                try:
                    ...
                    fee_paid_decimal = Decimal(
                        str(trade_fee_paid)) if trade_fee_paid else None
                    ...
                    logger.error(
                        f"Error processing order fill for {event.order_id}: {e}, skipping update")
                    return
                ...
                if order:
                    try:
                        ...
                        validated_fee = trade_fee_paid if trade_fee_paid and not math.isnan(
                            trade_fee_paid) else 0

                        # Use exchange_trade_id if available (unique per fill), fallback to generated id
                        exchange_trade_id = getattr(
                            event, 'exchange_trade_id', None)
                        ...
                        if result is None:
                            logger.debug(
                                f"Trade {trade_id} already exists, skipping duplicate")
                    except (ValueError, TypeError) as e:
                        logger.error(
                            f"Error creating trade record for {event.order_id}: {e}")
                        logger.error(
                            f"Trade data that failed: timestamp={event.timestamp}, "
                            f"amount={event.amount}, price={event.price}, fee={trade_fee_paid}")

            logger.debug(f"Recorded order fill: {event.order_id}")
        except Exception as e:
            logger.error(
                f"Error recording order fill for {event.order_id}: {e}")
    ...
    def _get_order_details_from_connector(self, order_id: str) -> Optional[dict]:
        ...
        try:
            if self._connector and hasattr(self._connector, 'in_flight_orders'):
                in_flight_order = self._connector.in_flight_orders.get(
                    order_id)
                if in_flight_order:
                    return {
        ...
        try:
            async with self.db_manager.get_session_context() as session:
                ...
                if existing_order:
                    ...
                    logger.info(
                        f"Updated existing order {event.order_id} to FAILED status")
                else:
                    # Try to get order details from connector's tracked orders
                    order_details = self._get_order_details_from_connector(
                        event.order_id)
                    if order_details:
                        logger.info(
                            f"Retrieved order details from connector for {event.order_id}: {order_details}")
                    ...
                    try:
                        await order_repo.create_order(order_data)
                        logger.info(
                            f"Created failed order record for {event.order_id}")
                        ...
                        if "duplicate key" in str(create_error).lower() or "unique constraint" in str(create_error).lower():
                            logger.info(
                                f"Order {event.order_id} already exists, updating status to FAILED")
                            await order_repo.update_order_status(
                                client_order_id=event.order_id,
                                status="FAILED",
    ...
    async def _handle_order_completed(self, event: Any):
        ...
        try:
            async with self.db_manager.get_session_context() as session:
                ...
                if order:
                    order.status = "FILLED"
                    order.exchange_order_id = getattr(
                        event, 'exchange_order_id', None)
                    await session.flush()
